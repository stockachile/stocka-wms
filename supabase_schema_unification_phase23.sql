-- WMS STOCKA - Supabase Schema Phase 23: Reinforced & Optimized Enviame Matching
-- Ejecuta este script en el SQL Editor de tu proyecto de Supabase.

-- 1. Crear función auxiliar de normalización de referencias
CREATE OR REPLACE FUNCTION public.clean_ref(p_ref TEXT, p_sigla TEXT)
RETURNS TEXT AS $$
DECLARE
  v_clean TEXT;
BEGIN
  ...
  -- Quitar espacios y caracteres iniciales no alfanuméricos (como #)
  v_clean := UPPER(regexp_replace(trim(p_ref), '^[^A-Z0-9]+', '', 'i'));
  
  -- Si el comercio posee sigla y la referencia inicia con ella, removerla
  IF p_sigla IS NOT NULL AND length(trim(p_sigla)) > 0 THEN
    p_sigla := UPPER(trim(p_sigla));
    IF v_clean LIKE p_sigla || '%' THEN
      v_clean := substring(v_clean from length(p_sigla) + 1);
      -- Volver a remover cualquier carácter no alfanumérico resultante (ej. SIM#1234 -> #1234 -> 1234)
      v_clean := regexp_replace(v_clean, '^[^A-Z0-9]+', '', 'i');
    END IF;
  END IF;
...
-- 2. Redefinir la función trigger para actualizar la orden al ingresar un envío de Envíame
...
BEGIN
  ...
  IF v_order_uuid IS NULL THEN
    ...
    IF v_comercio_name IS NOT NULL AND v_clean_order_id IS NOT NULL THEN
      -- 1. Intentar coincidencia exacta primero dentro del mismo comercio (aprovecha índice de external_order_number)
      -- Nota: Se hace JOIN con profiles porque orders.merchant_id contiene el UUID del perfil (auth.uid), no del comercio_config.
      SELECT o.id, o.status INTO v_order_uuid, v_current_wms_status
      FROM public.orders o
      JOIN public.profiles p ON p.id = o.merchant_id
      WHERE LOWER(trim(p.comercio)) = LOWER(trim(v_comercio_name))
...
-- 3. Redefinir la función trigger para actualizar la tabla consolidada envios_unificados
...
-- 4. Sincronización Retroactiva de Históricos OPTIMIZADA
-- Para evitar un timeout upstream en Supabase por actualizar miles de registros con triggers en una sola transacción,
-- ejecutamos la sincronización SOLAMENTE para aquellos envíos de Envíame que actualmente NO están asociados a ninguna orden.
UPDATE public.enviame_shipments
SET updated_at = timezone('utc', now())
WHERE id NOT IN (
  SELECT enviame_delivery_id 
  FROM public.orders 
  WHERE enviame_delivery_id IS NOT NULL
);
