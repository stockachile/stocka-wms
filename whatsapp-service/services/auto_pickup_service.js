...
async function autoProcessSinglePickupOrder(orderId, options = {}) {
  ...
  try {
    ...
    if (!wmsPickupExists) {
      ...
      const { error: wmsInsErr } = await wmsClient.from('store_pickups').insert([{
        id: nextId,
        comercio: order.comercio,
        pedido: orderNo,
      ...
      if (wmsInsErr) console.error('[AutoPickup] Error insertando en WMS store_pickups:', wmsInsErr.message);
    }
  } catch (e) {
  ...
  try {
    ...
    if (!pickerPickupExists) {
      const { error: pickerInsErr } = await pickerClient.from('sucursal_pickups').insert([{
        comercio: order.comercio,
        pedido: orderNo,
        nombre_apellido: order.customer_name,
        estado_pedido: 'NO PREPARADO',
      ...
      if (pickerInsErr) console.error('[AutoPickup] Error insertando en Picker sucursal_pickups:', pickerInsErr.message);
    }
  } catch (e) {
