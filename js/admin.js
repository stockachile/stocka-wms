              ...
              <div class="card-body" style="padding: 1.5rem;">
                <form id="form-sku-equivalence">
                  ...
                  <div class="form-group" style="margin-bottom: 1.25rem;">
                    ...
                    <small style="display:block; margin-top: 0.25rem; color: var(--color-text-muted);">Debe coincidir con un SKU del Catálogo del Cliente.</small>
                  </div>
            ...
            <div class="card" style="border: none; box-shadow: var(--shadow-md); margin:0;">
              ...
              <div class="card-body" style="padding: 1.5rem; display: flex; flex-direction: column; gap: 1rem;">
                <p style="margin: 0; color: var(--color-text-muted); font-size: 0.85rem; line-height: 1.5;">
                  Sube una planilla Excel o CSV con las equivalencias para el cliente seleccionado. Las columnas deben incluir: <strong style="color: var(--color-text-main);">Plataforma</strong>, <strong style="color: var(--color-text-main);">SKU Plataforma</strong>, y <strong style="color: var(--color-text-main);">SKU Master</strong>.
                </p>
                <div>
...
window.renderAdminSkuMappings = async function() {
  ...
  const comercio = window.activeAdminComercio;
  if (!comercio) return;
  ...
  try {
    ...
    window.currentAdminSkuMappings = mappings || [];
    renderAdminSkuMappingsTable(window.currentAdminSkuMappings);
...
function renderAdminSkuMappingsTable(mappings) {
  const tbody = document.getElementById('eq-table-body');
  if (!tbody) return;
...
function setupAdminSkuMappingsListeners() {
  ...
  if (searchInput) {
    ...
    newSearchInput.addEventListener('input', (e) => {
      ...
      if (!window.currentAdminSkuMappings) return;
      const filtered = window.currentAdminSkuMappings.filter(m => 
        m.platform_sku.toLowerCase().includes(term) || 
        m.master_sku.toLowerCase().includes(term) ||
        m.platform.toLowerCase().includes(term)
      );
      renderAdminSkuMappingsTableFiltered(filtered);
    });
  }
  ...
  if (form) {
    ...
    newForm.addEventListener('submit', async (e) => {
      ...
      if (!options.includes(masterSku)) {
        if (!confirm(`El SKU Master "${masterSku}" no se encontró en el Catálogo de este cliente. ¿Deseas guardarlo de todas formas? (Se recomienda que el SKU exista para evitar descuadres de stock).`)) {
          return;
        }
      }
...
function renderAdminSkuMappingsTableFiltered(mappings) {
  const tbody = document.getElementById('eq-table-body');
  if (!tbody) return;
