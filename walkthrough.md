...
3. **Carga y Registro de Órdenes de Ingreso en el Picker (`js/admin.js`)**:
   - Implementamos `window.sendIntakeToPicker(id)` para registrar los productos declarados en la tabla `active_orders` del Picker con el prefijo de orden `ING-${ID}` y agenda `'INGRESO'`.
    - [x] **Generación de Libro Excel Adicional**: Crea un libro XLSX multi-hoja (Resumen de Cobros, Detalle de Despachos y Detalles de Indemnización si existen) usando `XLSX` (SheetJS) al registrar la importación, guardándose en Supabase Storage junto con el PDF.
    - [x] **`processSelectedEnviameImports`**: Procesa la cola en lotes: genera y sube los PDFs individuales y Excels de cada sub-comercio, genera y sube adicionalmente el PDF Consolidado del Conglomerado para holdings de múltiples tiendas (guardándose en `enviame_pdfs` con `net_amount: 0` para no duplicar sumas de facturación), actualiza `billing_records` y gatilla los correos electrónicos si corresponde.
    - [x] **Persistencia de Histórico en Caliente (Supabase Storage)**: Al confirmar definitivamente la importación de un periodo, el sistema compila automáticamente un archivo JSON (`billing_files/${periodId}_enviame_confirmed_stats.json`) con las estadísticas definitivas recalculadas (incluyendo reasignaciones e indemnizaciones) y lo sube al bucket `payment_receipts` en Supabase. Al cargar el panel o previsualizar periodos futuros, el sistema lee los periodos registrados e integra en caliente estos datos con el histórico estático local, permitiendo actualizaciones de analíticas en tiempo real sin recargar la página.
