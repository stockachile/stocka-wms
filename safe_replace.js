...
// Find the backtick before the end string
const backtickIdx = content.lastIndexOf("`;", endIdx);
if (backtickIdx === -1) {
    console.error("No backtick match");
    process.exit(1);
}

// Find 'Integraciones Ecommerce'
const h2Idx = content.indexOf("Integraciones Ecommerce</h2>");
if (h2Idx === -1) {
    console.error("No h2 match");
    process.exit(1);
}

// Find the closest <div style="margin-bottom: 2rem;"> BEFORE the h2
const divStr = '<div style="margin-bottom: 2rem;">';
const startIdx = content.lastIndexOf(divStr, h2Idx);
...
console.log("Found boundaries:", {startIdx, h2Idx, backtickIdx, endIdx});
...
const newContent = content.substring(0, startIdx) + templateReplacement + content.substring(backtickIdx);
