...
async function replay() {
  console.log('=== VERIFICANDO PEDIDOS DESPACHADOS DE SIMPLEMENTE CAFE ===\n');
  ...
  console.log(`Analizando ${orders.length} pedidos...`);

  let skippedCount = 0;

  for (const o of orders) {
    if (o.status !== 'despachado') {
      console.log(`Pedido ${o.external_order_number} status is ${o.status}, skipping.`);
      continue;
    }
    ...
    if (!shouldProcess) {
      console.log(`Pedido ${o.external_order_number} no califica para procesar según inicio configurado.`);
      continue;
    }
    ...
    if (movs && movs.length > 0) {
      console.log(`Pedido ${o.external_order_number} ya tiene movimientos registrados.`);
      continue;
    }

    skippedCount++;
    console.log(`\n⚠️  PEDIDO SIN MOVIMIENTO DETECTADO:`);
    ...
    for (const item of items) {
      ...
      if (runFix) {
        ...
        const newQty = Math.max(0, (currentInv?.quantity || 0) - item.quantity);
        console.log(`  Cantidad actual: ${currentInv?.quantity || 0} -> Nueva cantidad: ${newQty}`);
