...
async function run() {
  console.log('Querying order with external_order_number 2000014156878607...');
  const { data: orders, error: ordersError } = await client
    .from('orders')
    .select('*')
    .eq('external_order_number', '2000014156878607');
  ...
  if (orders && orders.length > 0) {
    ...
    items.forEach((item, idx) => {
      ...
      console.log(`  warehouse_id: ${item.warehouse_id}`);
