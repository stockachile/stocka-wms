-- WMS STOCKA - Supabase Schema Phase 27: Clasificación de "pendiente - requiere solucion" en ALERTA para Envíame
-- Ejecuta este script en el SQL Editor de tu proyecto de Supabase.

-- 1. Actualizar función get_global_status para clasificar "requiere solucion" e incidencias de Envíame en ALERTA
...
BEGIN
  ...
  ELSIF source_table = 'enviame_shipments' THEN
    ...
    IF s LIKE '%requiere solucion%' OR s LIKE '%requiere solución%' OR s LIKE '%pendiente - requiere%'
       ...
       OR s LIKE '%cancel%' OR s LIKE '%rechazad%' THEN
      RETURN 'ALERTA';
...
-- 2. Recalcular y actualizar envíos unificados históricos de Envíame aplicando la nueva lógica
UPDATE public.envios_unificados
SET global_status = public.get_global_status(source_table, status)
WHERE source_table = 'enviame_shipments' OR id LIKE 'enviame_shipments:%';
